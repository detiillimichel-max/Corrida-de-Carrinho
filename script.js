(() => {
  ...
  function resetGame(){
    updateTrack();
    car = {
      t:-Math.PI/2, // angle around ellipse
      x:track.cx, y:track.cy-track.outerRy+track.width/2,
      angle:-Math.PI/2,
      vx:0, vy:0,
      w:36, h:56,
      drift:0,
  ...
  function spawnCoin(t){
    ...
    coins.push({x:p.x, y:p.y, t, r:14, taken:false, bob:Math.random()*Math.PI*2});
  }
  function spawnObstacle(t){
    const lane = 0.15+Math.random()*0.7;
    const p = pointOnTrack(t, lane);
    obstacles.push({x:p.x, y:p.y, t, w:34, h:34, type: Math.random()<0.6?'cone':'barrel'});
  }
  ...
  // Input
  const setSteer = ()=>{
    steerTarget = (rightDown?1:0) - (leftDown?1:0);
  };
      ...
      div.onclick = ()=>{
        if(!ownedFlag){
          if(totalCoins>=s.price){
            ...
            div.animate([{transform:'translateX(0)'},{transform:'translateX(-4px)'},{transform:'translateX(4px)'},{transform:'translateX(0)'}],{duration:300});
          }
        }else{
          currentSkin=s.id; persist(); renderShop(); updateHUD();
        }
      };
  ...
  function update(dt){
    ...
    // collisions
    // with track bounds (stay inside)
    // coins
    for(const c of coins){
      if(c.taken) continue;
      const pc = pointOnTrack(c.t,0.5);
      const dy = pc.y - camY;
      const dx = pc.x - car.x;
      if(Math.hypot(dx, dy - (car.y)) < 36){
    ...
    for(const o of obstacles){
      ...
      if(Math.abs(po.x - car.x)<30 && Math.abs(dy - car.y)<38){
        ...
        // game over after 3 hits? simplify: end
    ...
    if(distAhead - lastSpawnDist > 0.3){
      lastSpawnDist = distAhead;
      if(Math.random()<0.7) spawnCoin(car.t + 0.8 + Math.random()*0.5);
      if(Math.random()<0.4) spawnObstacle(car.t + 0.9 + Math.random()*0.4);
    }
  ...
  function render(){
    ...
    // sky gradient already via css, draw ground
    // Draw track
    ctx.save();
    ...
    drawEllipse(track.cx, track.cy, track.outerRx, track.outerRy);
    ctx.fill();

    // inner grass hole
    ctx.globalCompositeOperation='destination-out';
    drawEllipse(track.cx, track.cy, track.innerRx, track.innerRy);
    ctx.fill();
    ...
    drawEllipse(track.cx, track.cy, track.outerRx-4, track.outerRy-4); ctx.stroke();
    drawEllipse(track.cx, track.cy, track.innerRx+4, track.innerRy+4); ctx.stroke();

    // center dashed line
    ctx.setLineDash([18,18]);
    ctx.lineWidth=4;
    ctx.strokeStyle='rgba(255,255,255,.7)';
    drawEllipse(track.cx, track.cy, (track.innerRx+track.outerRx)/2, (track.innerRy+track.outerRy)/2);
    ctx.stroke();
    ctx.setLineDash([]);

    // start/finish line
    const sp = pointOnTrack(-Math.PI/2,0);
    const ep = pointOnTrack(-Math.PI/2,1);
    ctx.lineWidth=10;
    ctx.strokeStyle='#fff';
    ctx.beginPath(); ctx.moveTo(sp.x,sp.y); ctx.lineTo(ep.x,ep.y); ctx.stroke();
    ctx.strokeStyle='#000';
    ctx.setLineDash([10,10]); ctx.lineWidth=6;
    ctx.beginPath(); ctx.moveTo(sp.x,sp.y); ctx.lineTo(ep.x,ep.y); ctx.stroke();
    ctx.setLineDash([]);
    ...
    ctx.fillStyle='#222';
    skidMarks.forEach(s=>{ ctx.globalAlpha=s.a*0.25; ctx.beginPath(); ctx.arc(s.x, s.y+camY, 4,0,Math.PI*2); ctx.fill(); });
    ctx.globalAlpha=1;

    // coins
    coins.forEach(c=>{
      if(c.taken) return;
      const p=pointOnTrack(c.t,0.5);
      c.bob+=0.15;
      const by = Math.sin(c.bob)*4;
      drawEmoji('🪙', p.x, p.y+by, 28);
    });

    // obstacles
    obstacles.forEach(o=>{
      const p=pointOnTrack(o.t,0.5);
      drawEmoji(o.type==='cone'?'🚧':'🛢️', p.x, p.y, 34);
    });
    ...
    particles.forEach(p=>{
      ...
      ctx.beginPath(); ctx.arc(p.x, p.y+camY, 3,0,Math.PI*2); ctx.fill();
    });
    ctx.globalAlpha=1;
    ...
    ctx.rotate(car.angle + Math.PI/2 + steer*0.25*car.drift);
    // shadow
    ctx.globalAlpha=0.25; ctx.fillStyle='#000';
    ctx.beginPath(); ctx.ellipse(4,4,20,30,0,0,Math.PI*2); ctx.fill(); ctx.globalAlpha=1;
    // body
    const skin = skins.find(s=>s.id===currentSkin);
    drawEmoji(skin?.emoji||'🚗',0,0,48);
    // drift smoke
    if(car.drift>0.3){
      ctx.globalAlpha=0.4*car.drift;
      ctx.fillStyle='#fff';
      ctx.beginPath(); ctx.ellipse(-10,28,12,8,0,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(10,28,12,8,0,0,Math.PI*2); ctx.fill();
      ctx.globalAlpha=1;
    }
    ctx.restore();
